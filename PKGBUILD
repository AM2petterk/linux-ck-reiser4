...
# Contributor for Linux-ck: graysky <graysky AT archlinux DOT us>
# Thanks to: Edward Shishkin, for providing his Reiser4FS patchset to the public.
#            graysky for providing the GCC patchset and Linux-CK builds.
...
# Disable NUMA support (default=y). Changing this adds no additional benefits for a regular desktop system.
_disable_numa=y
...
# Note that this config has disabled btrfs, tablet and touchscreen support. 
...
license=(GPL2)
makedepends=(kmod inetutils bc libelf git lz4 gzip xz)
options=('!strip')
...
_package() {
  ...
  #pkgdesc="${_Kpkgdesc}"
  depends=(coreutils linux-firmware kmod mkinitcpio git lz4)
  optdepends=(
